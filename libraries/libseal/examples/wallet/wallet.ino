...
void setup() {
  ...
  ret = se_secure_storage_personalize(false);
  if (ret == SE_SUCCESS)
...
static  uint8_t key0[16] = {0xDD, 0xDD, 0xBF, 0xAC, 0x11, 0x70, 0x55, 0x9C, 0xC9, 0xB6, 0x28, 0x0F, 0x92, 0x95, 0xDF, 0x32};
static  uint8_t key1[16] = {0xEC, 0x05, 0xDD, 0xAC, 0x2F, 0xAA, 0x89, 0xC6, 0x1E, 0x77, 0x29, 0x37, 0x9A, 0x3A, 0xDF, 0xDE};
static  uint8_t key2[16] = {0xEE, 0xEE, 0x08, 0x55, 0x77, 0xBD, 0xA7, 0xB8, 0xA7, 0xAF, 0x58, 0xD1, 0x8B, 0x92, 0xF0, 0xDF};

memcpy(key00,key0,sizeof(key00));
memcpy(key01,key1,sizeof(key00));
memcpy(key02,key2,sizeof(key00));
